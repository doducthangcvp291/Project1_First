...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
        ...
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                if (!mContactHasChanged){
                    //hiển thị hộp thoại hỏi xem có lưu lại ko hay xóa bỏ khi nhấn back
                    // we havn't finished filling up some field
    ...
    private boolean saveContact() {
        //last step of this activity we have to create save contact method
        String name = mNameEditText.getText().toString().trim();
        String email = mEmailEditText.getText().toString().trim();
        String phone = mNumberEditText.getText().toString().trim();

        //what if wa haven't entered any text in field and we click save it will crash so how save it from crashing
